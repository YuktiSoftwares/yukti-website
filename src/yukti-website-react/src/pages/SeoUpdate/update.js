$(document).ready(function () {
  ...
  $.ajax({
    ...
    data: {
      ...
      postId: postId,
      siteurl: "https://yuktisoftware.com/update.html",
    },
    dataType: "json",
...
$(document).on("click", "#dealSave", function () {
  ...
  if (!name) {
    $("#dealName")
      .parent()
      .append('<small class="text-danger validation">Please enter name.</small>');
    flag = false;
  }
  if (!phone) {
    $("#dealPhone")
      .parent()
      .append('<small class="text-danger validation">Please enter phone number.</small>');
    flag = false;
  }
  if (unknownId == "") {
    $("#dealPhone")
      .parent()
      .append(
        '<small class="text-danger validation">OOPS! Something wrong, Please try after some time.</small>'
      );
    flag = false;
  }
  if (flag) {
    $.ajax({
      ...
      success: function (response) {
        if (response.status) {
          ...
          $("#messageMessage")
            .parent()
            .append('<small class="text-success validation">' + response.msg + "</small>");
        } else {
          if (response.msg.name) {
            $("#dealName")
              .parent()
              .append('<small class="text-danger validation">' + response.msg.name + "</small>");
          }
          if (response.msg.phone) {
            $("#dealPhone")
              .parent()
              .append('<small class="text-danger validation">' + response.msg.phone + "</small>");
          }
          if (response.msg.unknownId) {
            $("#messageMessage")
              .parent()
              .append(
                '<small class="text-danger validation">OOPS! Somethings wrong please try again.</small>'
              );
          }
        }
      },
...
$(document).on("click", "#leadSaveModal", function () {
  ...
  if (!name) {
    $("#plugin__enquery-name")
      .parent()
      .append('<small class="text-danger validation">Please enter name.</small>');
    flag = false;
  }
  if (!phone) {
    $("#plugin__enquery-phone")
      .parent()
      .append('<small class="text-danger validation">Please enter phone number.</small>');
    flag = false;
  }
  if (!message) {
    $("#plugin__enquery-message")
      .parent()
      .append('<small class="text-danger validation">Please enter message.</small>');
    flag = false;
  }
  if (unknownId == "") {
    $("#plugin__enquery-phone")
      ...
      success: function (response) {
        if (response.status) {
          ...
          $("#plugin__enquery-message")
            .parent()
            .append(
              '<small class="text-success validation" id="modal-succ">' + response.msg + "</small>"
            );
